...
Game::Game(const char *windowName, int windowWidth, int windowHeight) :
	m_windowName(windowName), m_windowWidth(windowWidth), m_windowHeight(windowHeight), m_world(flecs::world()) {
	// Raylib window
	...
	rendering::gui::GUIModule::init_prefabs(m_world);
	...
	m_world.entity("button 1")
		.set<rendering::gui::Button>({"Hello World",
		...
		.set<Rectangle>({200, 200, 300, 100});

	m_world.entity("text 0")
				  .set<rendering::gui::Text>({"Hello World"})
				  .set<Rectangle>({m_windowWidth / 2.f - 200, 0, 200, 300})
				  .set<rendering::gui::Outline>({2, GetColor(GuiGetStyle(LABEL, BORDER_COLOR_NORMAL)),
												 GetColor(GuiGetStyle(LABEL, BACKGROUND_COLOR))});

	auto t = m_world.entity("text 1")
				 .set<rendering::gui::Text>({"Hello World"})
				 .set<Rectangle>({m_windowWidth / 2.f, 0, 200, 300})
				 .set<rendering::gui::Outline>({2, GetColor(GuiGetStyle(LABEL, BORDER_COLOR_NORMAL)),
												GetColor(GuiGetStyle(LABEL, BACKGROUND_COLOR))});

	m_world.entity("deco 1").is_a(m_world.lookup(rendering::gui::DECORATOR_TEXT_ALIGN_CENTER)).child_of(t);

	auto t2 = m_world.entity("text 2")
				  .set<rendering::gui::Text>({"Hello World"})
				  .set<Rectangle>({m_windowWidth / 2.f + 200, 0, 200, 300})
				  .set<rendering::gui::Outline>({2, GetColor(GuiGetStyle(LABEL, BORDER_COLOR_NORMAL)),
												 GetColor(GuiGetStyle(LABEL, BACKGROUND_COLOR))});

	m_world.entity("deco 2").is_a(m_world.lookup(rendering::gui::DECORATOR_TEXT_ALIGN_RIGHT)).child_of(t2);
}
