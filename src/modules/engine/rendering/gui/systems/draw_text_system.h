...
namespace rendering::gui::systems {
    inline void draw_text_system(const Text &text, const Rectangle &rect, const FontAtlas &fonts) {
        Vector2 pos;
        ...
        DrawTextEx(fonts.fonts.at(text.font_size), text.text.c_str(), pos, text.font_size, 0, GUIModule::font_color());
    }
}
